 ...
 *
 * Exporta:
 * - `setupLabelRenderer`: Para configurar o renderizador de rótulos. (Não mais usado diretamente por ThreeScene após refatoração de setupRenderPipeline)
 * - `updateLabelRendererSize`: Para atualizar o tamanho em redimensionamentos.
 * - `updateAnnotationPins`: Para gerenciar os pins de anotação.
 */
import * as THREE from 'three';
import { CSS2DRenderer, CSS2DObject } from 'three/examples/jsm/renderers/CSS2DRenderer.js';
import type { Annotation, Equipment, Layer } from '@/lib/types';

// setupLabelRenderer não é mais exportado pois sua lógica foi integrada em setupRenderPipeline
// em scene-elements-setup.ts para centralizar a configuração dos renderizadores.
    ...
    annotations.forEach(anno => {
      ...
      if (equipmentForItem) {
        ...
        if (equipmentForItem.type === 'Tank' || equipmentForItem.type === 'Pipe') {
          ...
          yOffset = (equipmentForItem.radius || 0.3) + 0.8;
        }
