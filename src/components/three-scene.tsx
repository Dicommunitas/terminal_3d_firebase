...
interface ThreeSceneProps {
  ...
  selectedEquipmentTags: string[]; // Changed from selectedEquipmentId to selectedEquipmentTags
...
const ThreeScene: React.FC<ThreeSceneProps> = ({
  ...
  useEffect(() => {
    ...
    sceneRef.current.background = new THREE.Color(0xA9C1D1); // Sky blueish grey
    ...
    controlsRef.current.mouseButtons = {
      ...
      MIDDLE: THREE.MOUSE.ROTATE, // Middle mouse button now also rotates
    ...
    filteredEquipmentData.forEach(item => {
      ...
      if (existingMesh) {
        ...
        if (isVisibleByLayer && existingMesh instanceof THREE.Mesh) {
          // Re-create or update material based on colorMode
          ...
          if (newMesh.geometry) newMesh.geometry.dispose(); // dispose temporary geometry
          if (newMesh.material) (newMesh.material as THREE.Material).dispose(); // dispose temporary material
        }
      } else {
        if (isVisibleByLayer) {
  ...
  useEffect(() => {
    ...
    onSystemFramed();
  }, [targetSystemToFrame, onSystemFramed]);
